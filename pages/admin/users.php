...
$per_page = 15; // fixed per page, like peminjaman page

// --- Build WHERE clause ---
$where = [];
if ($status === 'active') $where[] = "is_active = 1";
else $where[] = "is_active = 0";
...
<head>
    ...
    <style>
        ...
        .btn-deactivate { background: transparent; border: none; color: #e74c3c; }
        .pager-btn.disabled, .pager-btn[aria-disabled="true"] { opacity: 0.5; pointer-events: none; }
    </style>
</head>
...
<div class="shell">
    ...
    <div class="main">
        ...
        <main class="content">
            <section class="hero">
                <div class="hero-text"><h1 class="hero-title">Pengguna</h1></div>
            </section>
            <div class="grid">
                <section class="col-12 card">
                        ...
                        <div class="data-toolbar">
                            <div class="data-toolbar-left">
                                <div class="input-icon" style="flex:1; max-width:320px;">
                                    ...
                                    <input class="input" type="search" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Cari nama, email, atau ID...">
                                </div>
                            </div>
                            <div class="data-toolbar-right">
                                <select class="select" name="role" style="width:auto; padding:7px 28px 7px 10px; font-size:12px;">
                                    <option value="">Semua role</option>
                                    <option value="admin" <?= $role==='admin' ? 'selected' : '' ?>>Admin</option>
                                    <option value="user"  <?= $role==='user'  ? 'selected' : '' ?>>User</option>
                                </select>
                                <select class="select" name="status" style="width:auto; padding:7px 28px 7px 10px; font-size:12px; margin-left:10px;">
                        ...
                        <table class="data-table" style="margin:0 22px; min-width:900px;">
                            ...
                            <tbody>
                                <?php if ($total_rows > 0 && mysqli_num_rows($result) > 0): ?>
                                    ...
                                    <?php endwhile; ?>
                                <?php else: ?>
                                    <tr><td colspan="5" class="text-center py-4 text-muted">Tidak ada data user <?= $status === 'active' ? 'aktif' : 'nonaktif' ?><?= ($search||$role) ? ' dengan filter yang dipilih.' : '' ?></td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div class="data-foot">
                        <div class="data-foot-info">
                            ...
                            ?>
                            <span>Showing <strong style="color:var(--t-base)"><?= $from_n ?>–<?= $to_n ?></strong> of <strong style="color:var(--t-base)"><?= $total_rows ?></strong></span>
                        </div>
                        <?php if ($total_pages > 1): ?>
                        <div class="pager">
                            <?php
                            // helper to build query string for pagination links
                            $qs = function($p) use ($search, $role, $status) {
                                $params = array_filter([
                                    'search' => $search,
                                    'role'   => $role,
                                    'status' => $status,
