    ...
    <div class="row align-items-center g-0">
      ...
      <div class="col-lg-5 col-md-12">
        <div class="hero-image-wrap">
          <img src="https://api.builder.io/api/v1/image/assets/TEMP/d5c06e7805b738c7332014e3ae976be268cf1209?width=1200" alt="Conference Room" loading="eager" class="img-fluid">
        </div>
      </div>
    </div>
  </div>
...
<script>
    // Smooth scroll untuk tombol "Lihat Ruangan"
    document.querySelector('.btn-hero-outline')?.addEventListener('click', function(e) {
        e.preventDefault();
        const target = document.querySelector('#daftar-ruangan');
        if (target) {
            target.scrollIntoView({ behavior: 'smooth' });
        }
    });

    // Konfirmasi reset filter (opsional)
    const resetLink = document.querySelector('.alert-info a');
    if (resetLink) {
    ...
    // Auto close alert setelah 5 detik (jika ada)
    const alertBox = document.querySelector('.alert-info');
    if (alertBox && !alertBox.classList.contains('no-autoclose')) {
        setTimeout(() => {
    ...
    // Optional: Tambahkan efek fade on scroll untuk card (bisa diabaikan)
