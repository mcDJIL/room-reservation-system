...
(function () {
  ...
  const isRoomsPage =
    ...
    (!!document.getElementById("add-building") &&
      !!document.getElementById("add-facility"));
  const isApprovalsPage =
    !!document.getElementById("add-user") ||
    !!document.getElementById("add-date");
  ...
  document.querySelectorAll(".btn-view").forEach((btn) =>
    btn.addEventListener("click", function () {
      ...
      // Tampilkan status berdasarkan data-active jika tersedia, fallback ke data-status
      var isActive = row.dataset.active;
  ...
  if (photoUploadBtn)
    photoUploadBtn.addEventListener("click", function (e) {
      ...
      if (!roomId) {
        alert("Room ID tidak ditemukan. Buka detail ruangan terlebih dahulu.");
        return;
      }
      ...
      fetch("../../actions/room/upload_photo.php", {
        method: "POST",
        body: fd,
      })
        .then((r) => r.json())
        .then((data) => {
          ...
          alert("Upload sukses");
          // Simple behavior: reload page to show new photos
          window.location.reload();
        })
        .catch((err) => {
          console.error(err);
        ...
        .finally(() => {
          ...
          photoUploadBtn.textContent = "Upload Foto";
        });
    });
  ...
  if (formEdit)
    formEdit.addEventListener("submit", function (e) {
      ...
      payload.append(
        "building",
        document.getElementById("e-building").value || "",
      );
      payload.append(
        "capacity",
        document.getElementById("e-capacity").value || "0",
      );
      payload.append(
        "status",
        document.getElementById("e-status").value || "Tidak Aktif",
      );
      ...
      fetch("../../actions/room/update.php", {
        ...
        .then((data) => {
          ...
          // perbarui dataset active numeric untuk detail modal
          ...
          if (buildingCell)
            buildingCell.textContent = room.building_name || "Tanpa Gedung";
          const capacityCell = row.querySelector("td:nth-child(3)");
          if (capacityCell)
            capacityCell.textContent = (room.capacity || 0) + " Orang";
          ...
          if (statusBadge) {
            ...
            statusBadge.classList.add(
              room.is_active == 1 ? "success" : "danger",
            );
            statusBadge.textContent =
              room.is_active == 1 ? "Aktif" : "Tidak Aktif";
          }
  ...
  document.querySelectorAll(".btn-delete").forEach((btn) =>
    btn.addEventListener("click", function () {
      ...
      document.getElementById("del-name").textContent = isApprovalsPage
        ? row.dataset.userName || row.dataset.name || ""
        : row.dataset.name || "";
