...
/**
 * Non-module wrapper for Chart.js wiring.
 * Loads against a global `Chart` (provided by vendor-chartjs.js) so the file
 * can be included with a plain `<script src=".../charts.js"></script>`.
 */

;(function () {
  function whenChartAvailable(cb) {
    if (window.Chart) return cb(window.Chart);
    let tries = 0;
    const id = setInterval(() => {
      if (window.Chart) {
        clearInterval(id);
        return cb(window.Chart);
      }
      if (++tries > 80) {
        clearInterval(id);
        console.warn('charts.js: Chart did not become available after waiting.');
      }
    }, 25);
  }

  whenChartAvailable((ChartGlobal) => {
    if (ChartGlobal.register && ChartGlobal.registerables) {
      try { ChartGlobal.register(...ChartGlobal.registerables); } catch (e) { /* ignore */ }
    }
...
function applyDefaults(t) {
  ChartGlobal.defaults.font.family = "'Inter', system-ui, sans-serif";
  ChartGlobal.defaults.font.size = 12;
  ChartGlobal.defaults.color = t.muted;
  ChartGlobal.defaults.borderColor = t.soft;
  ChartGlobal.defaults.plugins.legend.position = 'bottom';
  ChartGlobal.defaults.plugins.legend.labels.usePointStyle = true;
  ChartGlobal.defaults.plugins.legend.labels.padding = 16;
  ChartGlobal.defaults.plugins.legend.labels.boxWidth = 8;
  ChartGlobal.defaults.plugins.legend.labels.boxHeight = 8;
  ChartGlobal.defaults.plugins.tooltip.backgroundColor = t.text;
  ChartGlobal.defaults.plugins.tooltip.titleColor = t.bg;
  ChartGlobal.defaults.plugins.tooltip.bodyColor = t.bg;
  ChartGlobal.defaults.plugins.tooltip.padding = 10;
  ChartGlobal.defaults.plugins.tooltip.cornerRadius = 6;
  ChartGlobal.defaults.plugins.tooltip.displayColors = false;
}
  ...
  function buildAll() {
    ...
    document.querySelectorAll('canvas[data-chart-key]').forEach((canvas) => {
      ...
      instances.set(canvas, new ChartGlobal(canvas, seed(t)));
    });
  }
